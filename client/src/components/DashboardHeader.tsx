...
export default function DashboardHeader({
  ...
  return (
    <div className="flex flex-col md:flex-row border-b h-auto min-h-14 items-center justify-between bg-background/95 px-2 md:px-4 py-2 backdrop-blur supports-[backdrop-filter]:backdrop-blur sticky top-0 z-40 gap-2">
      {/* Left side: Logo + Trigger (mobile) */}
      <div className="flex items-center gap-2 w-full md:w-auto justify-between md:justify-start">
        <div className="flex items-center gap-2">
          {isMobile && <SidebarTrigger className="h-8 w-8 rounded-lg bg-background flex-shrink-0" />}
        ...
        {/* Filters on same line on mobile */}
          ...
          <div className="flex items-center gap-1.5">
            ...
            {selectedMonth !== undefined && onMonthChange && (
              <Select value={selectedMonth.toString()} onValueChange={(val) => onMonthChange(parseInt(val))}>
                <SelectTrigger className="w-[70px] h-8 text-[11px] px-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
            ...
            {selectedYear !== undefined && onYearChange && (
              <Select value={selectedYear.toString()} onValueChange={(val) => onYearChange(parseInt(val))}>
                <SelectTrigger className="w-[65px] h-8 text-[11px] px-2">
                  <SelectValue />
                </SelectTrigger>
                <SelectContent>
